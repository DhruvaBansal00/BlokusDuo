...
class MyPanelb extends JPanel implements ActionListener,MouseListener,KeyListener
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if(player1.getPlaced()>=21 || player2.getPlaced()>=21){
            endGame(g);
        }
        ...
        if(playerturn && !end){
            ...
            g.drawString("Player 1 Turn", 500, 450);
        }
        else if(!end){
            g.setColor(Color.red);
            g.setFont(new Font("Arial", Font.PLAIN, 28));
            g.drawString("Player 2 Turn", 500, 450);
        }
    }
    ...
    public void endGame(Graphics g){
        ...
        for(int i = 0; i<player1.playerPieces.length; i++){
            player1.playerPieces[i].remove();
            player2.playerPieces[i].remove();
        }

        if(player1.getScore() > player2.getScore()){
            g.setColor(Color.black);
            g.setFont(new Font("Arial", Font.PLAIN, 48));
            g.drawString("Player 1 Wins!", 500, 450);
        }

        if(player2.getScore() > player1.getScore()){
            g.setColor(Color.black);
            g.setFont(new Font("Arial", Font.PLAIN, 48));
            g.drawString("Player 2 Wins!", 500, 450);
        }
    }
    ...
    public void mouseReleased(MouseEvent e)
    {
        ...
        released.x+=15;
        released.y = released.y-30+15;
        if(piecePicked>-1)
        {
            int midSquare[] = getSquareIndex(released.x+15, released.y+15);
            if(midSquare[0]>-1 && midSquare[1]>-1)
            {
                if(player1pressed && playerturn)
                {
                    ...
                    for (int x = 0; x < 5; x++) {
                        for (int y = 0; y < 5; y++) {
                            if (player1.playerPieces[piecePicked].pieceIndex[x][y] > 0 && ((midSquare[0]-(2-y)+1 > -1 && midSquare[1]-(2-x)+1 > -1 && grid[midSquare[0] - (2 - y)+1][midSquare[1] - (2 - x) + 1].getColor() == 1) || (midSquare[0]-(2-y)+1 > -1 && midSquare[1]-(2-x)-1 > -1 &&grid[midSquare[0] - (2 - y)+1][midSquare[1] - (2 - x) - 1].getColor() == 1) || (midSquare[0]-(2-y)-1 > -1 && midSquare[1]-(2-x)+1 > -1 &&grid[midSquare[0] - (2 - y) -1][midSquare[1] - (2 - x) + 1].getColor() == 1) || (midSquare[0]-(2-y)-1 > -1 && midSquare[1]-(2-x)-1 > -1 &&grid[midSquare[0] - (2 - y)-1][midSquare[1] - (2 - x) - 1].getColor() == 1))) {
                                legal = true;
                                System.out.println("Corners:check");
                            }

                            if (player1.playerPieces[piecePicked].pieceIndex[x][y] > 0 && (midSquare[0] - (2 - y) + 1 >= grid.length || midSquare[0] - (2 - y) - 1 < 0 || midSquare[1] - (2 - x) + 3 >= grid.length || midSquare[1] - (2 - x) + 1 < 0)) {
                                legal = false;
                                break;
                            }
                            if (player1.playerPieces[piecePicked].pieceIndex[x][y] > 0 && (grid[midSquare[0] - (2 - y)][midSquare[1] - (2 - x) + 2].getColor() != 0 || grid[midSquare[0] - (2 - y) - 1][midSquare[1] - (2 - x) + 2].getColor() == 1 || grid[midSquare[0] - (2 - y) + 1][midSquare[1] - (2 - x) + 2].getColor() == 1 || grid[midSquare[0] - (2 - y)][midSquare[1] - (2 - x) + 3].getColor() == 1 || grid[midSquare[0] - (2 - y)][midSquare[1] - (2 - x) + 1].getColor() == 1)) {
                                legal = false;
                                break;
                            }
                    ...
                    if(legal) {
                        for (int x = 0; x < 5; x++) {
                            for (int y = 0; y < 5; y++) {
                                if (player1.playerPieces[piecePicked].pieceIndex[x][y] > 0) {
                                    grid[midSquare[0] - (2 - y)+1][midSquare[1] - (2 - x) + 2].setColor(1);
                                    player1.playerPieces[piecePicked].remove();
                                }
                            }
                        }
                        playerturn = !playerturn;
                        player1.changeScore(player1.playerPieces[piecePicked].getArrayList().size());
                        player1.placedPiece(1);
                    }
                    else{
                        player1.playerPieces[piecePicked].reset();
                    }
                }
                else if(!player1pressed && !playerturn)
                {
                    /*ArrayList<int[]> checkSquares = new ArrayList<int[]>();
                    for (int x = 0; x < 5; x++) {
                        for (int y = 0; y < 3; y++) {
                    ...
                    for (int x = 0; x < 5; x++) {
                        for (int y = 0; y < 5; y++) {
                            ...
                            if (player2.playerPieces[piecePicked].pieceIndex[x][y] > 0 && (midSquare[0] - (2 - y)+1>=grid.length || midSquare[0] - (2 - y)-1<0 || midSquare[1] - (2 - x) + 3>=grid.length || midSquare[1] - (2 - x) + 1<0)){
                                legal = false;
                                break;
                            }
                            if (player2.playerPieces[piecePicked].pieceIndex[x][y] > 0 && (grid[midSquare[0] - (2 - y)][midSquare[1] - (2 - x) + 2].getColor()!=0 || grid[midSquare[0] - (2 - y)-1][midSquare[1] - (2 - x) + 2].getColor()==2 || grid[midSquare[0] - (2 - y)+1][midSquare[1] - (2 - x) + 2].getColor()==2 || grid[midSquare[0] - (2 - y)][midSquare[1] - (2 - x) + 3].getColor()==2 || grid[midSquare[0] - (2 - y)][midSquare[1] - (2 - x) + 1].getColor()==2)) {
                                legal = false;
                            }
                        }
                    ...
                    if(legal) {
                        for (int x = 0; x < 5; x++) {
                            for (int y = 0; y < 5; y++) {
                                if (player2.playerPieces[piecePicked].pieceIndex[x][y] > 0) {
                                    grid[midSquare[0] - (2 - y)+1][midSquare[1] - (2 - x) + 2].setColor(2);
                                    player2.playerPieces[piecePicked].remove();
                                }
                            }
                        }
                        playerturn = !playerturn;
                        player2.changeScore(player2.playerPieces[piecePicked].getArrayList().size());
                        player2.placedPiece(1);
                    }
                    else{
                        player2.playerPieces[piecePicked].reset();
                    }
                }
            }
